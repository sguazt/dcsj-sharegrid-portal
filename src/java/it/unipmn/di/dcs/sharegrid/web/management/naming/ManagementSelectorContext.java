...
public class ManagementSelectorContext implements Context {
     ...
     */
    public ManagementSelectorContext(Hashtable env) {
        this.env = env;
    }
     ...
     * Builds a Catalina selector context using the given environment.
     */
    public ManagementSelectorContext(Hashtable env, boolean initialContext) {
        this(env);
     ...
     */
    protected StringManager sm = StringManager.getManager( ManagementSelectorContext.class.getPackage().getName() );
    ...
    protected Context getBoundContext() throws NamingException {
        ...
        if (initialContext) {
            ...
            if (ManagementContextBindings.isThreadBound()) {
                ICName += ManagementContextBindings.getThreadName();
            } else if (ManagementContextBindings.isClassLoaderBound()) {
                ICName += ManagementContextBindings.getClassLoaderName();
            }
            Context initialContext = ManagementContextBindings.getContext(ICName);
            if (initialContext == null) {
                // Allocating a new context and binding it to the appropriate 
                // name
                initialContext = new ManagementNamingContext(env, ICName);
                ManagementContextBindings.bindContext(ICName, initialContext);
            }
            return initialContext;
        } else {
            if (ManagementContextBindings.isThreadBound()) {
                return ManagementContextBindings.getThread();
            } else {
                return ManagementContextBindings.getClassLoader();
            }
        }
