 ...
 */
@Experimental("This filter can be used to implement a centralized access control mechanism (e.g. for each page check if current user has enough permissions to access)")
@Deprecated
public class SecurityFilter implements Filter
{
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException
	{
		...
		if ( session != null && request.getRequestedSessionId() != null && session.getAttribute( this.userIdAttr ) != null )
		{
			...
			if ( this.isPathAccessibleByUser(path, user) )
			{
				forbidden = false;
			}
		}
		else if ( this.isPathAccessibleByAnonymous(path) )
		{
			forbidden = false;
		}
